...
{
    /* No heap resources inside ChunkManager beyond the struct itself.
     * This stub makes the shutdown contract explicit; add real cleanup here
     * if pool slots or other owned resources are introduced later. */
    (void)cm;
}
                                 ...
                                 int tile_size, float time)
{
    ...
    chunk_blit(chunk, renderer, off_x, off_y, tile_size);
    chunk_render_objects(chunk, renderer, off_x, off_y, tile_size);
    chunk_render_foliage(chunk, renderer, off_x, off_y, tile_size, time);
...
int chunk_manager_obj_solid_at(ChunkManager *cm, int tile_x, int tile_y)
{
    ...
    uint8_t oid = chunk->obj_id[local_r][local_c];
    if (oid == 0) return 0;
    return obj_is_solid((ObjectType)chunk->objects[oid - 1].type);
}
