...
int main(int argc, char *argv[])
{
    ...
    /* Linear filtering so PNG tiles scale smoothly at intermediate zoom levels. */
    ...
    if (tile_sprites_init(renderer) != 0) {
    ...
    while (running) {
        ...
        /* Normalise diagonal movement so it matches cardinal speed. */
        if (dx != 0.0f && dy != 0.0f) { dx *= 0.707107f; dy *= 0.707107f; }

        /* Block movement into water tiles or solid world objects.
         * Each axis is tested independently so the player slides along walls
         * instead of stopping dead on a corner hit.
         * Bounding box: a square centred on the player, PLAYER_HALF tile-units
         * per side, scaled to current pixel tile_size. */
        if (dx != 0.0f || dy != 0.0f) {
            float half = PLAYER_HALF * (float)cam.tile_size;
            float ts   = (float)cam.tile_size;

            /* Test X axis against current Y position. */
            if (dx != 0.0f) {
                float nx = player.wx + dx * dt;
                float cx0[4] = {nx - half, nx + half, nx - half, nx + half};
                float cy0[4] = {player.wy - half, player.wy - half,
                                player.wy + half, player.wy + half};
                for (int i = 0; i < 4; i++) {
                    int tx = (int)floorf(cx0[i] / ts);
                    int ty = (int)floorf(cy0[i] / ts);
                    TileType t = chunk_manager_tile_at(cm, tx, ty);
                    if (tile_is_water(t) || chunk_manager_obj_solid_at(cm, tx, ty)) {
                        dx = 0.0f; break;
                    }
                }
            }

            /* Test Y axis against current X position. */
            if (dy != 0.0f) {
                float ny = player.wy + dy * dt;
                float cx1[4] = {player.wx - half, player.wx + half,
                                player.wx - half, player.wx + half};
                float cy1[4] = {ny - half, ny - half, ny + half, ny + half};
                for (int i = 0; i < 4; i++) {
                    int tx = (int)floorf(cx1[i] / ts);
                    int ty = (int)floorf(cy1[i] / ts);
                    TileType t = chunk_manager_tile_at(cm, tx, ty);
                    if (tile_is_water(t) || chunk_manager_obj_solid_at(cm, tx, ty)) {
                        dy = 0.0f; break;
                    }
